...
export default function Novedades() {
  return (
    <section className="relative w-full bg-[#1a1a1a] pt-24 pb-12 lg:pt-32 lg:pb-16 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="text-center mb-10 sm:mb-12 lg:mb-16">
          <motion.h2 
            ...
            viewport={{ once: true }}
            className="font-serif text-3xl sm:text-4xl lg:text-5xl text-sand-100 mb-3 sm:mb-4"
          >
            Novedades
          </motion.h2>
          <motion.div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {news.map((item, index) => (
            <motion.div 
              ...
              className="group bg-charcoal-900/40 backdrop-blur-sm border border-warm-gold-400/10 overflow-hidden hover:border-warm-gold-400/50 transition-colors duration-300 flex flex-col shadow-sm hover:shadow-md"
            >
              <div className="relative h-48 overflow-hidden">
                <div className="absolute inset-0 bg-charcoal-900/20 group-hover:bg-transparent transition-colors duration-500 z-10" />
                <img 
                  src={item.image} 
                  alt={item.title} 
                ...
                />
                <div className="absolute top-4 left-4 z-20 bg-charcoal-900/95 backdrop-blur-md px-3 py-1 border border-warm-gold-400/30">
                  <span className="font-sans text-[10px] uppercase tracking-widest text-warm-gold-400 font-bold">
                    {item.category}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 flex flex-col flex-grow">
                <span className="font-sans text-xs text-sand-100/50 mb-3 block">
                  {item.date}
                </span>
                <h3 className="font-serif text-xl text-sand-100 mb-3 group-hover:text-warm-gold-400 transition-colors">
                  {item.title}
                </h3>
                <p className="font-sans text-sm text-sand-100/70 mb-6 flex-grow">
                  {item.description}
                </p>
                <button className="flex items-center gap-2 font-sans text-xs uppercase tracking-widest text-warm-gold-400 group-hover:text-warm-gold-300 transition-colors w-fit">
